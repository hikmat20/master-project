...
if (!empty($id)) {
    ...
    $name_supplier = $this->db->get_where('master_supplier', array('id_supplier' => $id))->row();
}
            ...
            <table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
                <tbody>
                    <tr style='background-color: #175477; font-size: 15px;'>
                        ...
                        <th class="">
                            <label for=""><?= $name_supplier->nm_supplier_office ?></label>
                            <input type="hidden" name="id_supplier" id="id_supplier" class="required" value="<?= $id ?>">
                            <label class="label label-danger hideIt id_supplier">Pattern Name Can't be empty!</label>
                        </th>
                    </tr>
                    <tr style='background-color: #175477; font-size: 15px;'>
    ...
    $(document).ready(function() {
        ...
        if ('<?= empty($getC->id_pattern) ?>') {
            ...
            var row = '<option value=""></option>' +
                <?php foreach ($getS as $key => $vs) : ?> '<option value="<?= $vs->id_supplier ?>"><?= $vs->nm_supplier_office ?></option>' +
                <?php endforeach; ?> '';
