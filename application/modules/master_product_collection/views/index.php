  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '#add_photo', function() {
      if (parseInt($(".list-photo").length == 0)) {
        ...
        var no = parseInt($(".list-photo").length + 1);
      }
      ...
      if (no <= 15) {
        html = '' +
          ...
          '<td>' +
          '<button type="button" data-id="' + no + '" class="btn btn-sm btn-primary" onclick=$("#photo_' + no + '").click()><i class="fa fa-upload"></i> Upload</button>' +
          ...
          '</td>' +
          '<td><img id="preview_' + no + '" class="text-left" style="height: 50px;margin: 0px auto;width: auto;"></td>' +
          '<td><input type="text" name="name_photo[]" class="form-control" placeholder="Name Photo"</td>' +
          '<td><button class="hapus_photo btn btn-sm btn-danger">x</button></td>' +
          '</tr>';
        $('#data-photo tbody').append(html)

      } else {
        swal({
          title: "Sorry!",
          text: "Maksimal 15 photo .",
          type: "error",
          timer: 2500,
