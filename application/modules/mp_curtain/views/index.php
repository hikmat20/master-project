<?php
    $ENABLE_ADD     = has_permission('Mp_curtain.Add');
    $ENABLE_MANAGE  = has_permission('Mp_curtain.Manage');
    $ENABLE_VIEW    = has_permission('Mp_curtain.View');
    $ENABLE_DELETE  = has_permission('Mp_curtain.Delete');
?>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.edit', function(e){
      ...
      $("#view").load(siteurl+active_controller+'/modal_Process/Curtain/edit/'+id);
      $("#ModalView").modal();
		});
    ...
    $(document).on('click', '.delete', function(e){
      var id_curtain = $(this).data('id_curtain');
      swal({
        title: "Are you sure?",
    ...
    jQuery(document).on('click', '#addPatternNameSave', function(){
      ...
      $.ajax({
        url: siteurl+"master_curtain/savePatternName",
        dataType : "json",
        type: 'POST',
        data: formdata,
        processData:false,
    ...
    $(document).on('click', '#selBrand', function(e){
      ...
      var id_sup = $(this).data('id_curtain');
      //console.log(id_sup);
    ...
    $(document).on('click', '#addBrandSave', function(e){
      ...
      $.ajax({
        url: siteurl+"master_curtain/saveBrand",
        dataType : "json",
        type: 'POST',
        data: formdata,
        success: function(result){
    ...
    $(document).on('click', '#addPatternName', function(e){
      ...
      $("#view2").load(siteurl+'/master_curtain/modal_Process/PatternName/add/');
      $("#ModalView2").modal();
    });
    ...
    $(document).on('click', '#form-product #addBC', function(e){
      $("#ModalView2 .modal-dialog").css('width','65%');
      $("#head_title2").html("<b>Add Basic Component</b>");
      $("#view2").load(siteurl+active_controller+'/modal_Process/basicComponent/add/');
      $("#ModalView2").modal();
    });
    jQuery(document).on('click', '#addCurtainSave', function(){
      /*if ($(".type_accesories").val() == 'Component') {
        $("#NonComponent").html('');
      }else {
        $("#Component").html('');
      }*/
      //console.log($('.form-active').html());
      var formdata = new FormData($(".form-active")[0]);
      console.log(formdata);
      var valid = true;//getValidation();
      ...
      if (valid) {
        ...
        console.log(formdata);
        $.ajax({
          url: siteurl+active_controller+"saveCurtain",
          dataType : "json",
          type: 'POST',
          data: formdata,
          processData:false,
    ...
    jQuery(document).on('click', '#form-type #addBasicSave', function(){
      //var x = parseInt(document.getElementById("tfoot-pic").rows.length)+1;
      //console.log(x);
      var code      = $('#id_component_1').val()+"-"+$('#id_component_2').val()+"-"+$('#id_component_3').val();
      var name      = $('#name_component').val();
      var price_set = parseFloat($('#buy_price').val());
      var price_pcs = parseFloat($('#buy_price_set').val());
      var qty       = parseFloat($('#qty_set').val());
      var uom       = $('#uom').val();
      if (qty == '' || qty == 0) {
        qty = 1;
      }
      ...
      var row = ''+
        '<tr class="addjs">'+
            '<td style="background-color:#E0E0E0">'+code+'<input type="hidden" name="id_component[]" value="'+code+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center">'+name+' ('+uom+') <input type="hidden" name="name_component[]" value="'+name+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center"> Rp '+price+' <input type="hidden" name="price_component[]" value="'+price+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center"> @ '+qty+' <input type="hidden" name="qty_component[]" value="'+qty+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center">'+uom+' <input type="hidden" name="uom_component[]" value="'+uom+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center"> Rp <span class="subt">'+total+'</span><input type="hidden" name="subt_component[]" value="'+total+'"></td>'+
            '<td style="background-color:#E0E0E0;text-align:center">'+
            '<a class="text-red hapus_item_js" href="javascript:void(0)" title="Delete List"><i class="fa fa-times"></i></a>'+
            '<a class="text-blue edit_item_js" href="javascript:void(0)" title="Edit List"><i class="fa fa-edit"></i></a>'+
            '</td>'+
        '</tr>'

      $('#basicComponent_Detail').append(row);
    });
  ...
  function DataTables(set=null){
    ...
    var dataTable3 = $('#table_curtain_type').DataTable({
      "serverSide": true,
      "stateSave" : true,
      "bAutoWidth": true,
      "destroy": true,
  ...
  function add_data(){
    $(".modal-dialog").css('width','95%');
    $("#head_title").html("<b>Add Curtain</b>");
    $("#view").load(siteurl+active_controller+'/modal_Process/Curtain/add/');
    $("#ModalView").modal();
  }
