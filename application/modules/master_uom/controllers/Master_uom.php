...
class Master_uom extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Master_uom.View';
    protected $addPermission  = 'Master_uom.Add';
    protected $managePermission = 'Master_uom.Manage';
    protected $deletePermission = 'Master_uom.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Master_uom/Uom_model',
                                 'Aktifitas/aktifitas_model',
                                ));
        $this->template->title('Manage Data UOM');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_uom'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['name_uom'])."</div>";
    			if($this->auth->restrict($this->viewPermission) ) :
            $nestedData[]	= "<div style='text-align:center'>

              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_uom='".$row['id_uom']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='detail btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_uom = '".$row['id_uom']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
          ...
          ".$where_activation."
  				AND (
  				id_uom LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR name_uom LIKE '%".$this->db->escape_like_str($like_value)."%'
  	        )
  		";
  		...
  		$sql .= " ORDER BY id_uom ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function saveUOM(){
      ...
      $counter = ($this->db->get('master_uom')->num_rows())+1;

      $this->db->trans_begin();
      if ($data['type'] == 'edit') {
        $id_uom = $data['id_uom'];
        $insertData	= array(
          'name_uom'	=> strtoupper($data['name_uom']),
          'modified_on'	=> date('Y-m-d H:i:s'),
          'modified_by'	=> $this->auth->user_id()
        );
        $this->db->where('id_uom',$data['id_uom'])->update('master_uom',$insertData);
      }else {
        $id_uom = "U".str_pad($counter, 3, "0", STR_PAD_LEFT);
        $insertData	= array(
          'id_uom'    => $id_uom,
          'name_uom'	=> strtoupper($data['name_uom']),
          'activation'  => "active",
        ...
        $this->db->insert('master_uom',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['type'].' UOM Data '.$id_uom;
        $status = 0;
        ...
        $keterangan = 'SUCCESS, '.$data['type'].' UOM Data '.$id_uom;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
    ...
    public function deleteUOM($id){
  		$this->db->trans_begin();
      $this->db->where('id_uom',$id)->delete('master_uom');
      $this->db->trans_complete();
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'Delete fail for UOM Data '.$id;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $this->db->trans_commit();
        $Arr_Kembali	= array(
          'pesan'		=>'Success delete uom item. Thanks.',
          'status'	=> 1
        );

        $keterangan = 'SUCCESS, delete UOM Data '.$id;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
