 ...
 *
 * This is controller for Master Product Class
 */

class Mp_class extends Admin_Controller
{
  //Permission
  protected $viewPermission = 'Mp_class.View';
  protected $addPermission  = 'Mp_class.Add';
  protected $managePermission = 'Mp_class.Manage';
  protected $deletePermission = 'Mp_class.Delete';
  ...
  public function getDataJSON()
  {
    ...
    foreach ($query->result_array() as $row) {
      ...
      $nestedData[]  = "<div align='left'>" . strtoupper($row['id_class']) . "</div>";
      $nestedData[]  = "<div align='left'>" . strtoupper($row['name_class']) . "</div>";
      $nestedData[]  = "<div align='center'>" . $row['value_sale'] . "</div>";
      if ($this->auth->restrict($this->viewPermission)) :
        $nestedData[]  = "<div style='text-align:center'>

              <!--<a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_class='" . $row['id_class'] . "' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-list'></span>
              </a>-->
              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_class='" . $row['id_class'] . "' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_class = '" . $row['id_class'] . "'  style='width:30px; display:inline-block'>
  ...
  public function queryDataJSON($activation, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "
  			SELECT
  				a.*
  			FROM
  				master_product_class a
  			WHERE 1=1
          " . $where_activation . "
  				AND (
  				a.id_class LIKE '%" . $this->db->escape_like_str($like_value) . "%'
  				OR a.name_class LIKE '%" . $this->db->escape_like_str($like_value) . "%'
  	        )
  		";
    ...
    $sql .= " ORDER BY a.id_class ASC, " . $columns_order_by[$column_order] . " " . $column_dir . " ";
  ...
  public function deleteData()
  {
    ...
    $this->db->trans_begin();
    $getCat   = $this->db->where('id_class', $id_class)->update('master_product_class', array('activation' => 'inactive', 'id_class' => 'Del-' . $id_class));
    ...
    if ($this->db->trans_status() === FALSE) {
      ...
      $keterangan = 'FAILED, Delete Customer Data ' . $id_class;
      $status = 0;
      ...
      $keterangan = 'SUCCESS, Delete Customer Data ' . $id_class;
      $status = 1;
      $nm_hak_akses = $this->addPermission;
      $kode_universal = $this->auth->user_id();
      $jumlah = 1;
  ...
  public function modal_Process($page = "", $action = "", $id = "")
  {
    ...
    if ($page == 'Class') {
      $this->template->render('modal_Process_Class');
    }
  }
  ...
  public function saveClass()
  {
    ...
    if ($data['type'] == 'edit') {
      ...
      $this->db->where('id_class', $data['id_class'])->update('master_product_class', $insertData);
    } else {
      $numID = $this->db->get_where('master_product_class', array('id_class' => $id_class))->num_rows();
      if ($numID > 0) {
        $getI   = $this->db->query("SELECT MAX(id_class) FROM master_product_class ORDER BY id_class DESC LIMIT 1")->row();
        //echo "$first_letter";
        //exit;
        $num = substr($getI->id_class, -5) + 1;
      ...
      $this->db->insert('master_product_class', $insertData);
    }
    ...
    if ($this->db->trans_status() === FALSE) {
      ...
      $keterangan = 'FAILED, ' . $data['type'] . ' Customer Data ' . $id_class . ' With Name ' . $name_class;
      $status = 0;
      $nm_hak_akses = $this->addPermission;
      $kode_universal = $this->auth->user_id();
      $jumlah = 1;
      ...
      $keterangan = 'SUCCESS, ' . $data['type'] . ' Brand Data ' . $id_class . ' With Name ' . $name_class;
      $status = 1;
      $nm_hak_akses = $this->addPermission;
      $kode_universal = $this->auth->user_id();
      $jumlah = 1;
