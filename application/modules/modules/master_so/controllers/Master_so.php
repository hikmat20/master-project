...
class Master_so extends Admin_Controller
{
  //Permission
  protected $viewPermission = 'Master_so.View';
  protected $addPermission  = 'Master_so.Add';
  protected $managePermission = 'Master_so.Manage';
  protected $deletePermission = 'Master_so.Delete';
  ...
  public function index()
  {
    ...
    $this->template->title('Quotation Deal');
    ...
    foreach ($query->result_array() as $row) {
      ...
      if ($this->auth->restrict($this->viewPermission)) :
        $nestedData[]  = "<div style='text-align:center'>
              <a class='btn btn-sm btn-danger print' href='javascript:void(0)' title='Print PDF' data-id_quotation='" . $row['id_quotation'] . "' style='width:30px; display:inline-block'>
                <span class='fa fa-file-pdf-o'></span> 
              </a>
              <a class='btn btn-sm btn-warning edit' href='javascript:void(0)' title='Edit' data-id_quotation='" . $row['id_quotation'] . "' style='width:30px; display:inline-block'>
                <span class='fa fa-edit'></span>
              </a>
              <a class='btn btn-sm btn-info detail' href='javascript:void(0)' title='Detail' data-id_quotation='" . $row['id_quotation'] . "' style='width:30px; display:inline-block'>
                <span class='fa fa-quote-right'></span>
              </a>
              <a class='btn btn-sm btn-primary upload' href='javascript:void(0)' title='Detail' data-id_quotation='" . $row['id_quotation'] . "' style='width:30px; display:inline-block'>
  ...
  public function saveUpdloadPo()
  {
    ...
    if (!empty($data)) {
      ...
      if (!$this->upload->do_upload('file_po_upload')) {
        ...
        $paths = $_SERVER['DOCUMENT_ROOT'] . 'assets/po/' . $name_file;
  ...
  public function print_quotation($id)
  {
    ...
    $show = $this->template->load_view('print_quotation', $data);
