<?php
    $ENABLE_ADD     = has_permission('Installation.Add');
    $ENABLE_MANAGE  = has_permission('Installation.Manage');
    $ENABLE_VIEW    = has_permission('Installation.View');
    $ENABLE_DELETE  = has_permission('Installation.Delete');
?>
                  ...
                  <thead>
                    <tr>
                      <th width="5">#</th>
                			...
                			<th>Name Installation</th>
                			<th>Time</th>
                			<th width="10%" class="text-center">Qty Man</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.edit', function(e){
      var id = $(this).data('id_install');
      $(".modal-dialog").css('width','60%');
      $("#head_title").html("<b>EDIT INSTALLATION COST</b>");
      $("#view").load(siteurl+active_controller+'/modal_Process/Installation/edit/'+id);
      $("#ModalView").modal();
		});
    ...
    $(document).on('click', '.detail', function(e){
      var id = $(this).data('id_install');
      $(".modal-dialog").css('width','60%');
      $("#head_title").html("<b>DETAIL INSTALLATION PRICE</b>");
      $("#view").load(siteurl+active_controller+'modal_Process/Installation/view/'+id);
      $("#ModalView").modal();
		});
//--------------------------------------------------------------------------------------------
    ...
    $(document).on('click', '.delete', function(e){
      var id_install = $(this).data('id_install');
      swal({
        title: "Are you sure?",
    ...
    jQuery(document).on('click', '#saveSewing', function(){
      var valid = getValidation();
      // console.log(valid);
      if (valid) {
        var formdata = new FormData(document.getElementById("form-installation"));//$("#form-supplier").serialize();
        // console.log(formdata);
        $.ajax({
          url: siteurl+active_controller+"saveDataInstall",
          dataType : "json",
          type: 'POST',
          data: formdata,
          processData:false,
  ...
  function add_data(){
    $(".modal-dialog").css('width','60%');
    $("#head_title").html("<b>Add Installation Cost</b>");
    $("#view").load(siteurl+active_controller+'/modal_Process/Installation/add/');
    $("#ModalView").modal();
  }
