...
class Blind extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Blind.View';
    protected $addPermission  = 'Blind.Add';
    protected $managePermission = 'Blind.Manage';
    protected $deletePermission = 'Blind.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Blind/Blind_model',
                                 'Aktifitas/aktifitas_model',
                                ));
        $this->template->title('Manage Blind');
        $this->template->page_icon('fa fa-navicon');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_blind'].'-'.$row['id_blind_2'].'-'.$row['id_blind_3'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['name_blind'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['id_supplier'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['color_code'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['fabric_code'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['width'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['height'])."</div>";
    			$nestedData[]	= "<div align='center'>".strtoupper($row['uom'])."</div>";
    			$nestedData[]	= "<div align='center'>".($row['remark'])."</div>";
    			$nestedData[]	= "<div align='right'>".(number_format($row['sale_price']))."</div>";
				...
				$nestedData[]	= "<div style='text-align:center'>
				  <a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_blind='".$row['id_blind']."' style='width:30px; display:inline-block'>
					<span class='glyphicon glyphicon-list'></span>
				  </a>
				  <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_blind='".$row['id_blind']."' style='width:30px; display:inline-block'>
					<span class='glyphicon glyphicon-edit'></span>
				  </a> 
				  <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_blind = '".$row['id_blind']."' style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
		...
		$sql = "SELECT a.* FROM blind a WHERE 1=1
				".$where_activation." 
				AND (
  				a.id_blind LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR a.status LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.id_supplier LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.name_blind LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.uom LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.color_code LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.fabric_code LIKE '%".$this->db->escape_like_str($like_value)."%'
  	        )
			";
  		...
  		$sql .= " ORDER BY a.id_blind ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function getID(){
      ...
      $getI   			= $this->db->query("SELECT * FROM blind WHERE LEFT(id_blind,2) = '$first_letter' ORDER BY id_blind DESC LIMIT 1")->row();
      // echo "$first_letter";
      // exit;
      $num = substr($getI->id_blind,2,2)+1;
    ...
    public function modal_Process($action="",$id=""){
      ...
      $this->template->render('modal_blind');
  	}
    ...
    public function saveDataBlind(){
		$data                 = $this->input->post();
		$type                 = $data['type'];
		
		$path           = './assets/img/blind/';
		$id         = $data['id_blind'];
		$config = array(
			  'upload_path' => './assets/img/blind/',
		...
		$this->upload->initialize($config);
		  ...
		  if ( !$this->upload->do_upload('image')){
			...
			$nm_img 		= 'blind_'.$data['id_blind'].".".$ext;
		  }
      ...
      if ($data['type'] == 'edit') {
        ...
        $insertData	= array(
			...
			// 'id_blind'          => $data['id_blind'], 
			'id_blind_2'        => $data['id_blind_2'], 
			'id_blind_3'        => $data['id_blind_3'], 
			'name_blind'        => $data['name_blind'], 
			...
			'height_chain'      => $data['height_chain'], 
			'position_chain'    => $data['posisi'], 
			'remark'            => $data['descrip'], 
        ...
        $this->db->where('id_blind',$data['id_blind'])->update('blind',$insertData);
      }else {
        ...
        $this->db->insert('blind',$insertData);
      }
    ...
    public function deleteData(){
      $id = $this->input->post('id_blind');
      $this->db->trans_begin();
      $getCat   = $this->db->where('id_blind',$id)->update('blind', array('activation'=>'nonaktif'));
      $this->db->trans_complete();
