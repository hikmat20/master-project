...
class Mp_accesories extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Mp_accesories.View';
    protected $addPermission  = 'Mp_accesories.Add';
    protected $managePermission = 'Mp_accesories.Manage';
    protected $deletePermission = 'Mp_accesories.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Mp_accesories/Mp_accesories_model',
                                 'Aktifitas/aktifitas_model',
                                ));
        $this->template->title('Manage Accesories Data');
    ...
    public function index()
    {
        ...
        $this->template->title('Accesories');
    ...
    public function getDataJSON(){
      ...
      $where_activation = "";
  		...
  		$sql = "
  			SELECT
  				a.*, b.nm_supplier, c.name_pattern_type
  			FROM
  				master_product_accesories a
          ...
          ".$where_activation."
  				...
  				OR c.name_pattern_type LIKE '%".$this->db->escape_like_str($like_value)."%'
    ...
    public function modal_Process($page="",$action="",$id=""){
      ...
      if ($page == 'Accesories') {
        $this->template->render('modal_Process_Accesories');
    ...
    public function saveAccesories(){
      ...
      ##UPLOAD
        ...
        $path           =   './assets/img/master_accesories/'; //path folder

        $config = array(
                'upload_path' => './assets/img/master_accesories/',
        ...
        if ( ! $this->upload->do_upload('product_photo')){
          ...
          $paths = $_SERVER['DOCUMENT_ROOT'].'assets/img/master_accesories/'.$name_pic;
      ...
      */
      ...
      //FABRIC DATA
      if ($data['type'] == 'edit') {
        $this->db->where('id_product',$data['id_product'])->update('master_product_accesories',$insertData);
      }else {
        $numID = $this->db->get_where('master_product_accesories', array('id_product'=>$id_product))->num_rows();
        if ($numID > 0) {
          $Arr_Kembali	= array(
          ...
          $this->db->insert('master_product_accesories',$insertData);
        }
      }
    ...
    public function deleteData(){
      ...
      $this->db->trans_begin();
      $getCat   = $this->db->where('id_product',$id_product)->update('master_product_accesories', array('activation'=>'nonaktif'));
      $this->db->trans_complete();
