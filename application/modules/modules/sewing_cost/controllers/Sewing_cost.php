...
class Sewing_cost extends Admin_Controller
{ 
    //Permission
    protected $viewPermission = 'Sewing_cost.View';
    protected $addPermission  = 'Sewing_cost.Add';
    protected $managePermission = 'Sewing_cost.Manage';
    protected $deletePermission = 'Sewing_cost.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Sewing_cost/Sewing_cost_model',
    ...
    public function index()
    {
        ...
        $this->template->title('Sewing Cost');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".($row['id_sewing_cost'])."</div>";
            ...
            $nestedData[]	= "<div style='text-align:center'>
              <a class='btn btn-sm btn-warning detail' href='javascript:void(0)' title='Detail' data-id_sewing_cost='".$row['id_sewing_cost']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-search'></span>
              </a>
              <a class='btn btn-sm btn-info edit' href='javascript:void(0)' title='Edit' data-id_sewing_cost='".$row['id_sewing_cost']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_sewing_cost = '".$row['id_sewing_cost']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
		...
		$sql = "SELECT
				a.*
				FROM
				  sewing_cost a WHERE 1=1
				".$where_activation." 
				AND (
  				a.id_sewing_cost LIKE '%".$this->db->escape_like_str($like_value)."%'
  		...
  		$sql .= " ORDER BY a.id_sewing_cost ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function getID(){
      ...
      $getI   			= $this->db->query("SELECT * FROM sewing_cost WHERE LEFT(id_sewing_cost,3) = '$first_letter' ORDER BY id_sewing_cost DESC LIMIT 1")->row();
      //echo "$first_letter";
      //exit;
      $num = substr($getI->id_sewing_cost,3,3)+1;
    ...
    public function saveFabricCost(){
      ...
      if ($data['type'] == 'edit') {
        ...
        $insertData	= array(
		  // 'id_sewing_cost'		=> $data['id_sewing_cost'],
        ...
        $this->db->where('id_sewing_cost',$data['id_sewing_cost'])->update('sewing_cost',$insertData);
      }else {
        $insertData	= array(
      	  'id_sewing_cost'		=> $data['id_sewing_cost'],
		  'nm_item' 			=> $data['nm_item'],
        ...
        $this->db->insert('sewing_cost',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['type'].' Process Cost Data '.$id_sewing_cost;
        $status = 0;
    ...
    public function deleteData(){
      $id_sewing_cost		= $this->input->post('id_sewing_cost');
      $this->db->trans_begin();
      $getCat   = $this->db->where('id_sewing_cost',$id_sewing_cost)->update('sewing_cost', array('activation'=>'nonaktif'));
      $this->db->trans_complete();
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, Delete Process Cost Data '.$id_sewing_cost;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $keterangan = 'SUCCESS, Delete Process Cost Data '.$id_sewing_cost;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
