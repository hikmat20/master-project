 ...
 *
 * This is controller for Master Product CARE INSTRUCTION
 */

class Mp_ci extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Mp_ci.View';
    protected $addPermission  = 'Mp_ci.Add';
    protected $managePermission = 'Mp_ci.Manage';
    protected $deletePermission = 'Mp_ci.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Mp_ci/Mp_ci_model',
                                 'Aktifitas/aktifitas_model',
                                ));
        $this->template->title('Manage Data Ci');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_ci'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['name_ci'])."</div>";
    			$nestedData[]	= "<div align='left'><img src='".base_url('/assets/img/care_instruction/'.$row['image_ci'])."' width='100' height='100'></div>";
    			if($this->auth->restrict($this->viewPermission) ) :
            $nestedData[]	= "<div style='text-align:center'>

              <!--<a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_ci='".$row['id_ci']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-list'></span>
              </a>-->
              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_ci='".$row['id_ci']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_ci = '".$row['id_ci']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
          ...
          ".$where_activation."
  				AND (
  				a.id_ci LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR a.name_ci LIKE '%".$this->db->escape_like_str($like_value)."%'
  	        )
  		";
  		...
  		$sql .= " ORDER BY a.id_ci ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function modal_Process($page="",$action="",$id=""){
      ...
      if ($page == 'CI') {
        $this->template->render('modal_Process_Ci');
      }
  	}
    ...
    public function saveCi(){
      ...
      //IDENTITY
      $id_ci      =$data['id_ci'];
      $name_ci    =$data['name_ci'];
      $activation         ='active';


      if ($data['type'] == 'edit') {
        $path = $_SERVER['DOCUMENT_ROOT'].'assets/img/care_instruction/'.$name_ci;
        unlink($path);
      }
      $path           =   './assets/img/care_instruction/'; //path folder
      $config = array(
              'upload_path' => './assets/img/care_instruction/',
      ...
      if ( ! $this->upload->do_upload('photo_ci')){
          $result = $this->upload->display_errors();
      }else{

        $data_foto  = array('upload_data' => $this->upload->data('photo_ci'));
        $ext = end((explode(".", $_FILES["photo_ci"]["name"])));
        $name_pic = $name_ci.".".$ext;
      }
      //echo $name_pic;
      //exit;
      $this->db->trans_begin();
      ...
      if ($data['type'] == 'edit') {
        ...
        $this->db->where('id_ci',$data['id_ci'])->update('master_product_ci',$insertData);
      }else {
        $numID = $this->db->get_where('master_product_ci', array('id_ci'=>$id_ci))->num_rows();
        if ($numID > 0) {
          $getI   = $this->db->query("SELECT MAX(id_ci) FROM master_product_ci ORDER BY id_ci DESC LIMIT 1")->row();
          //echo "$first_letter";
          //exit;
          $num = substr($getI->id_ci,-5)+1;
          $id_ci = 'img_'.str_pad($num,5,"0",STR_PAD_LEFT);
        }
        ...
        $this->db->insert('master_product_ci',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['type'].' Customer Data '.$id_ci.' With Name '.$name_ci;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $keterangan = 'SUCCESS, '.$data['type'].' Brand Data '.$id_ci.' With Name '.$name_ci;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
