...
class Accessories_cost extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Accessories_cost.View';
    protected $addPermission  = 'Accessories_cost.Add';
    protected $managePermission = 'Accessories_cost.Manage';
    protected $deletePermission = 'Accessories_cost.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Accessories_cost/Accessories_cost_model',
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".($row['id_accessories_cost'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['nm_item'])."</div>";
    			$nestedData[]	= "<div align='left'>".ucfirst($row['category_accessories'])."</div>";
    			$nestedData[]	= "<div align='left'>".($row['calculation'])."</div>";
    			$nestedData[]	= "<div align='center'>".($row['rate_persen'] == '0'?'-':$row['rate_persen']. ' %'). "</div>";
    			$nestedData[]	= "<div align='right'>".($row['rate_value'] == '0'?'-':number_format($row['rate_value'])). "</div>";
            ...
            $nestedData[]	= "<div style='text-align:center'>
              <a class='btn btn-sm btn-warning detail' href='javascript:void(0)' title='Detail' data-id_accessories_cost='".$row['id_accessories_cost']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-search'></span>
              </a>
              <a class='btn btn-sm btn-info edit' href='javascript:void(0)' title='Edit' data-id_accessories_cost='".$row['id_accessories_cost']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_accessories_cost = '".$row['id_accessories_cost']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
		...
		$sql = "SELECT
				a.*
				FROM
				  accessories_cost a WHERE 1=1
				".$where_activation." 
				AND (
  				a.id_accessories_cost LIKE '%".$this->db->escape_like_str($like_value)."%'
  		...
  		$sql .= " ORDER BY a.id_accessories_cost ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function getID(){
      ...
      $getI   			= $this->db->query("SELECT * FROM accessories_cost WHERE LEFT(id_accessories_cost,3) = '$first_letter' ORDER BY id_accessories_cost DESC LIMIT 1")->row();
      //echo "$first_letter";
      //exit;
      $num = substr($getI->id_accessories_cost,3,3)+1;
    ...
    public function saveAccessoriesCost(){
      ...
      if ($data['type'] == 'edit') {
        ...
        $insertData	= array(
		  // 'id_accessories_cost'		=> $data['id_accessories_cost'],
		  'nm_item' 			=> $data['nm_item'],
		  'calculation' 		=> $data['calculation'],
		  'rate_persen' 		=> str_replace(',','',$data['rate_persen']),
		  'rate_value' 			=> str_replace(',','',$data['rate_value']),
		  'category_accessories' => $data['category'],
		  'status' 				=> $data['status'],
          'modified_on'	        => date('Y-m-d H:i:s'),
          'modified_by'	        => $this->auth->user_id()
        );
        $this->db->where('id_accessories_cost',$data['id_accessories_cost'])->update('accessories_cost',$insertData);
      }else {
        $insertData	= array(
      	  'id_accessories_cost'		=> $data['id_accessories_cost'],
		  'nm_item' 			=> $data['nm_item'],
        ...
        $this->db->insert('accessories_cost',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['id_accessories_cost'].' Process Cost Data '.$id_accessories_cost;
        $status = 0;
        ...
        $keterangan = 'SUCCESS, '.$data['id_accessories_cost'].' Process Cost Data '.$id_accomodation;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
    ...
    public function deleteData(){
      $id_accessories_cost		= $this->input->post('id_accessories_cost');
      $this->db->trans_begin();
      $getCat   = $this->db->where('id_accessories_cost',$id_accessories_cost)->update('accessories_cost', array('activation'=>'nonaktif'));
      $this->db->trans_complete();
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, Delete Process Cost Data '.$id_accessories_cost;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $keterangan = 'SUCCESS, Delete Process Cost Data '.$id_accessories_cost;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
