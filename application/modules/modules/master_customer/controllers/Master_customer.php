...
class Master_customer extends Admin_Controller
{
    ...
    public function saveCustomer()
    {
        ...
        $ArrBranch = array();
        $no = 0;
        foreach ($data['pic_customer'] as $pic => $x) {
            $no++;
            $ArrPic[$pic]['id_pic']         = $data['id_customer'] . '-P' . str_pad($no + 1, 2, "0", STR_PAD_LEFT);
            $ArrPic[$pic]['name_pic']       = $x['name_pic'];
            $ArrPic[$pic]['id_customer']    = $data['id_customer'];
            $ArrPic[$pic]['phone_pic']      = $x['pic_phone'];
            $ArrPic[$pic]['email_pic']      = $x['pic_email'];
            $ArrPic[$pic]['position_pic']   = $x['pic_position'];
            $ArrPic[$pic]['religion_pic']   = $x['pic_religion'];
            $ArrPic[$pic]['npwp_pic']       = $x['npwp_pic'];
            $ArrPic[$pic]['bank_account']   = $x['bank_account'];
            $ArrPic[$pic]['birthday']       = $x['birthday'];


            $id_pic = $data['id_customer'] . '-P' . str_pad($no + 1, 2, "0", STR_PAD_LEFT);
            ...
            $config = array(
                'upload_path' => './assets/img/name_card_pic/',
            ...
            if (!$this->upload->do_upload('name_card')) {
                $result = $this->upload->display_errors();
            } else {
                $ext = end((explode(".", $_FILES['name_card']['name'])));
                $name_file = $id_pic . "." . strtolower($ext);
                $paths = $_SERVER['DOCUMENT_ROOT'] . 'assets/img/name_card_pic/' . $name_file;
                if (file_exists($paths)) {
                    unlink($paths);
                }
                $ArrPic[$pic]['name_card']      = $name_file;
                $data_foto  = array('upload_data' => $this->upload->data('name_card'));
            }
        }
