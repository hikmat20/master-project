...
class Master_product_collection extends Admin_Controller
{
  ...
  public function saveCollection()
  {
    ...
    // echo "<pre>";
    // // print_r($data);
    // print_r($_FILES['photo_collection']['name']);
    // echo "<pre>";
    // exit;
    $files_tmp = $_FILES;
    for ($i = 0; $i < count($files_tmp['photo_collection']['name']); $i++) {
      $_FILES['photo_collection']['name'] = $files_tmp['photo_collection']['name'][$i];
      $_FILES['photo_collection']['type'] = $files_tmp['photo_collection']['type'][$i];
      $_FILES['photo_collection']['tmp_name'] = $files_tmp['photo_collection']['tmp_name'][$i];
      $_FILES['photo_collection']['error'] = $files_tmp['photo_collection']['error'][$i];
      $_FILES['photo_collection']['size'] = $files_tmp['photo_collection']['size'][$i];

      $config = array(
        'upload_path' => './assets/img/collections/',
        'allowed_types' => 'gif|jpg|png|jpeg|JPG|PNG',
        'file_name' => $data['id_collection'] . "-" . str_pad($i + 1, 2, "0", STR_PAD_LEFT),
      ...
      $ext = end((explode(".", $files_tmp['photo_collection']['name'][$i])));
      $size = end((explode(".", $files_tmp['photo_collection']['size'][$i])));
      $size_x = round($size / 1024);

      $this->load->library('upload', $config);
      $this->upload->initialize($config);
      if (!$this->upload->do_upload('photo_collection')) {
        $result = $this->upload->display_errors();
      } else {

        $data_foto  = array('upload_data' => $this->upload->data('photo_collection'));
        $ext = end((explode(".", $files_tmp['photo_collection']['name'][$i])));
        $name_pic = $data['id_collection'] . "-" . str_pad($i + 1, 2, "0", STR_PAD_LEFT) . "." . strtolower($ext);
        $paths = $_SERVER['DOCUMENT_ROOT'] . '/assets/img/collections/' . $name_pic;
        if (file_exists($paths)) {
          unlink($paths);
        }
      }

      if (empty($files_tmp['photo_collection']['name'][$i])) {
        $dataPhoto[$i]['photo_file'] = $data['photo_old'][$i];
      } else {
        $dataPhoto[$i]['photo_file'] = $name_pic;
      }
      $dataPhoto[$i]['id_photo']       = $data['id_collection'] . "-" . str_pad($i + 1, 2, "0", STR_PAD_LEFT);
      $dataPhoto[$i]['id_collection']  = $data['id_collection'];
      $dataPhoto[$i]['name_photo']     = $data['name_photo'][$i];
    }
    ...
    if ($data['type'] == 'edit') {
      ...
      $this->db->delete('photo_collection', ['id_collection' => $id_collection]);
    } else {
    ...
    // if (!empty($dataPhoto)) {
    $this->db->insert_batch('photo_collection', $dataPhoto);
    // }
