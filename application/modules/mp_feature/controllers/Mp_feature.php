 ...
 *
 * This is controller for Master Product Feature
 */

class Mp_feature extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Mp_feature.View';
    protected $addPermission  = 'Mp_feature.Add';
    protected $managePermission = 'Mp_feature.Manage';
    protected $deletePermission = 'Mp_feature.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Mp_feature/Mp_feature_model',
                                 'Aktifitas/aktifitas_model',
                                ));
        $this->template->title('Manage Data Feature');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_feature'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['name_feature'])."</div>";
    			if($this->auth->restrict($this->viewPermission) ) :
            $nestedData[]	= "<div style='text-align:center'>

              <!--<a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_feature='".$row['id_feature']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-list'></span>
              </a>-->
              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_feature='".$row['id_feature']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_feature = '".$row['id_feature']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
  		...
  		$sql = "
  			SELECT
  				a.*
  			FROM
  				master_product_feature a
  			WHERE 1=1
          ".$where_activation."
  				AND (
  				a.id_feature LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR a.name_feature LIKE '%".$this->db->escape_like_str($like_value)."%'
  	        )
  		";
  		...
  		$sql .= " ORDER BY a.id_feature ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function modal_Process($page="",$action="",$id=""){
      ...
      if ($page == 'Feature') {
        $this->template->render('modal_Process_Feature');
      }
  	}
    ...
    public function saveFeature(){
      ...
      if ($data['type'] == 'edit') {
        ...
        $this->db->where('id_feature',$data['id_feature'])->update('master_product_feature',$insertData);
      }else {
        $numID = $this->db->get_where('master_product_feature', array('id_feature'=>$id_feature))->num_rows();
        if ($numID > 0) {
          $getI   = $this->db->query("SELECT MAX(id_feature) FROM master_product_feature ORDER BY id_feature DESC LIMIT 1")->row();
          //echo "$first_letter";
          //exit;
          $num = substr($getI->id_feature,-5)+1;
          $id_feature = 'F'.str_pad($num,5,"0",STR_PAD_LEFT);
        }
        ...
        $this->db->insert('master_product_feature',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['type'].' Customer Data '.$id_feature.' With Name '.$name_feature;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $keterangan = 'SUCCESS, '.$data['type'].' Brand Data '.$id_feature.' With Name '.$name_feature;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
