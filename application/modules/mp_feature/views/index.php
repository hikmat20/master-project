<?php
    $ENABLE_ADD     = has_permission('Mp_feature.Add');
    $ENABLE_MANAGE  = has_permission('Mp_feature.Manage');
    $ENABLE_VIEW    = has_permission('Mp_feature.View');
    $ENABLE_DELETE  = has_permission('Mp_feature.Delete');
?>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.edit', function(e){
      var id = $(this).data('id_feature');
      $(".modal-dialog").css('width','55%');
      $("#head_title").html("<b>EDIT FORM</b>");
      $("#view").load(siteurl+active_controller+'/modal_Process/Feature/edit/'+id);
      $("#ModalView").modal();
		});
    ...
    $(document).on('click', '.detail', function(e){
      ...
      $("#view").load(siteurl+active_controller+'modal_Process/Feature/view/'+id);
      $("#ModalView").modal();
		});
    ...
    jQuery(document).on('click', '#addFeatureSave', function(){
      ...
      if (valid) {
        ...
        $.ajax({
          url: siteurl+active_controller+"saveFeature",
          dataType : "json",
          type: 'POST',
          data: formdata,
          success: function(result){
  ...
  function add_data(){
    $(".modal-dialog").css('width','55%');
    $("#head_title").html("<b>Add Feature</b>");
    $("#view").load(siteurl+active_controller+'/modal_Process/Feature/add/');
    $("#ModalView").modal();
  }
