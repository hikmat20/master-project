...
{
    //Permission
    protected $viewPermission = 'Delivery.View';
    protected $addPermission  = 'Delivery.Add';
    protected $managePermission = 'Delivery.Manage';
    protected $deletePermission = 'Delivery.Delete';
    ...
    public function __construct()
    {
        ...
        $this->template->title('Manage Delivery Cost');
    ...
    public function index()
    {
        ...
        $this->template->title('Delivery Cost');
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_delivery'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['nm_prov'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['nm_kab'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['via'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['supplier'])."</div>";
            ...
            $nestedData[]	= "<div style='text-align:center'>
              <a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_delivery='".$row['id_delivery']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-list'></span>
              </a>
              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_delivery='".$row['id_delivery']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_delivery = '".$row['id_delivery']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
		...
		$sql = "SELECT
				a.*,b.nama nm_prov,c.nama nm_kab
				FROM
				  delivery a LEFT JOIN provinsi b on a.id_provinsi = b.id_prov 
				  LEFT JOIN kabupaten c ON a.id_kabupaten = c.id_kab WHERE 1=1
				".$where_activation." 
				AND (
  				a.id_delivery LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR a.status LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.via LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.supplier LIKE '%".$this->db->escape_like_str($like_value)."%'
  		...
  		$sql .= " ORDER BY a.id_delivery ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function getID(){
      ...
      $getI   			= $this->db->query("SELECT * FROM delivery WHERE LEFT(id_delivery,2) = '$first_letter' ORDER BY id_delivery DESC LIMIT 1")->row();
      //echo "$first_letter";
      //exit;
      $num = substr($getI->id_delivery,2,5)+1;
    ...
    public function modal_Process($page="",$action="",$id=""){
      ...
      if ($page == 'Delivery') {
        $this->template->render('modal_Process_Delivery');
      }
  	}
    ...
    public function addDelivery(){
		$this->template->title('Add Delivery Cost');
        $this->template->page_icon('fa fa-plus');
  		$this->template->render('form_add_delivery');
  	}
	
    public function editDelivery($id=''){
		$this->template->title('Edit Delivery Cost');
      ...
      if ($data['type'] == 'edit') {
        ...
        $insertData	= array(
          ...
          'modified_by'	        => $this->auth->user_id()
			...
			foreach($cost as $cs => $x) {
				
					$insert_cost[$cs]['id_delivery'] 	= $data['id_delivery'];
					$insert_cost[$cs]['weight_1']		= $x['weight1'];
					$insert_cost[$cs]['weight_2']		= $x['weight2'];
					$insert_cost[$cs]['item_cost']		= str_replace(",","",$x['item_cost']);
					$insert_cost[$cs]['created_by']   	= $this->auth->user_id();
					$insert_cost[$cs]['created_on']    	= date('Y-m-d H:i:s');
			
			}
		
        $this->db->where('id_delivery',$data['id_delivery'])->update('delivery',$insertData);
		$this->db->delete('delivery_cost',array('id_delivery' => $data['id_delivery']));
		$this->db->insert_batch('delivery_cost',$insert_cost);
        ...
        $insertData	= array(
		  ...
		  // $next_cost = array(
			...
			// 'weight_1'	  	=> $x['weight2'],
			// 'weight_2'	  	=> $x['weight2'],
			// 'item_cost'	  	=> str_replace(",","",$data['next_item_cost']),
			// 'created_on'    => date('Y-m-d H:i:s'),
			// 'created_by'    => $this->auth->user_id()
		  // );
		...
		$this->db->insert_batch('delivery_cost',$insert_cost);
		// $this->db->insert('delivery_cost',$next_cost);
      }
