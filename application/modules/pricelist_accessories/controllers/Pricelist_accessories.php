...
class Pricelist_accessories extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Pricelist_accessories.View';
    protected $addPermission  = 'Pricelist_accessories.Add';
    protected $managePermission = 'Pricelist_accessories.Manage';
    protected $deletePermission = 'Pricelist_accessories.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Pricelist_accessories/Pricelist_accessories_model',
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
            ...
            $nestedData[]	= "<div style='text-align:center'>
              ...
              <a class='btn btn-sm btn-info edit' href='javascript:void(0)' title='Edit' data-id_product='".$row['id_product']."' style='width:30px; display:inline-block'>
    ...
    public function savePricelist(){
      ...
      if ($data['type'] == 'edit') {
        ...
        $this->db->where('id_product',$data['id_product1'])->update('pricelist_fabric',$insertData);
      }else {
    ...
    public function approveData(){
	  ...
	  if ($data->status == '1') {
		  ...
		  $this->db->trans_begin();
		  $getCat   = $this->db->where('id_product',$id_product)->update('pricelist_fabric', array('status'=>'1'));
		  $this->db->trans_complete();
		  if($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			$Arr_Kembali	= array(
