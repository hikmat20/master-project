 ...
 *
 * This is controller for Master Product Content
 */

class Mp_content extends Admin_Controller
{
    //Permission
    protected $viewPermission = 'Mp_content.View';
    protected $addPermission  = 'Mp_content.Add';
    protected $managePermission = 'Mp_content.Manage';
    protected $deletePermission = 'Mp_content.Delete';
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Mp_content/Mp_content_model',
    ...
    public function getDataJSON(){
          ...
          $requestData['activation'],
    		...
    		foreach($query->result_array() as $row)
    		{
    			...
    			$nestedData[]	= "<div align='left'>".strtoupper($row['id_content'])."</div>";
    			$nestedData[]	= "<div align='left'>".strtoupper($row['name_content'])."</div>";
    			if($this->auth->restrict($this->viewPermission) ) :
            $nestedData[]	= "<div style='text-align:center'>

              <!--<a class='btn btn-sm btn-primary detail' href='javascript:void(0)' title='Detail' data-id_content='".$row['id_content']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-list'></span>
              </a>-->
              <a class='btn btn-sm btn-success edit' href='javascript:void(0)' title='Edit' data-id_content='".$row['id_content']."' style='width:30px; display:inline-block'>
                <span class='glyphicon glyphicon-edit'></span>
              </a>
              <a class='btn btn-sm btn-danger delete' href='javascript:void(0)' title='Delete' data-id_content = '".$row['id_content']."'  style='width:30px; display:inline-block'>
      ...
      $where_activation = "";
  		...
  		$sql = "
  			SELECT
  				a.*
  			FROM
  				master_product_content a
  			WHERE 1=1
          ".$where_activation."
  				AND (
  				a.id_content LIKE '%".$this->db->escape_like_str($like_value)."%'
  				OR a.name_content LIKE '%".$this->db->escape_like_str($like_value)."%'
  	        )
  		";
  		...
  		$sql .= " ORDER BY a.id_content ASC, ".$columns_order_by[$column_order]." ".$column_dir." ";
    ...
    public function deleteData(){
      ...
      $this->db->trans_begin();
      $getCat   = $this->db->where('id_content',$id_content)->update('master_product_content', array('activation'=>'inactive','id_content'=>'Del-'.$id_content));
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, Delete Customer Data '.$id_content;
        $status = 0;
        ...
        $keterangan = 'SUCCESS, Delete Customer Data '.$id_content;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
    ...
    public function modal_Process($page="",$action="",$id=""){
      ...
      if ($page == 'Content') {
        $this->template->render('modal_Process_Content');
      }
  	}
    ...
    public function saveContent(){
      ...
      $name_content    =$data['name_content'];
      $activation         ='active';
      ...
      if ($data['type'] == 'edit') {
        ...
        $this->db->where('id_content',$data['id_content'])->update('master_product_content',$insertData);
      }else {
        $numID = $this->db->get_where('master_product_content', array('id_content'=>$id_content))->num_rows();
        if ($numID > 0) {
          $getI   = $this->db->query("SELECT MAX(id_content) FROM master_product_content ORDER BY id_content DESC LIMIT 1")->row();
          //echo "$first_letter";
          //exit;
          $num = substr($getI->id_content,-5)+1;
        ...
        $this->db->insert('master_product_content',$insertData);
      }
      ...
      if($this->db->trans_status() === FALSE){
        ...
        $keterangan = 'FAILED, '.$data['type'].' Customer Data '.$id_content.' With Name '.$name_content;
        $status = 0;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
        ...
        $keterangan = 'SUCCESS, '.$data['type'].' Brand Data '.$id_content.' With Name '.$name_content;
        $status = 1;
        $nm_hak_akses = $this->addPermission;
        $kode_universal = $this->auth->user_id();
        $jumlah = 1;
